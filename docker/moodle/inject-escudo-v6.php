...
/* CRÍTICO: sin esto el ::after se posiciona relativo al body */
#mooveslideshow {
...
.tau-r1 { animation: tcw  20s linear infinite; transform-origin: 100px 100px; }
.tau-r2 { animation: tccw 28s linear infinite; transform-origin: 100px 100px; }
.tau-r3 { animation: tpulse 4s ease-in-out infinite; }
.tau-nd { animation: tnd 2.8s ease-in-out infinite alternate; }
.tau-nd:nth-child(2) { animation-delay: -.7s; }
.tau-nd:nth-child(3) { animation-delay: -1.4s; }
.tau-nd:nth-child(4) { animation-delay: -2.1s; }
.tau-nw { animation: tnw 3.4s ease-in-out infinite alternate; }
.tau-nw:nth-child(2) { animation-delay: -.85s; }
.tau-nw:nth-child(3) { animation-delay: -1.7s; }
.tau-nw:nth-child(4) { animation-delay: -2.55s; }

@keyframes tcw   { to { transform: rotate(360deg);  } }
@keyframes tccw  { to { transform: rotate(-360deg); } }
@keyframes tpulse{ 0%,100%{opacity:.22} 50%{opacity:.55} }
@keyframes tnd   { from{opacity:.45} to{opacity:1} }
@keyframes tnw   { from{opacity:.35} to{opacity:.85} }
...
(function(){
  function tau_init(){
    ...
    /* Aros SVG alrededor del escudo (::after) */
    if (!ss.querySelector(".tau-rings-wrap")) {
      var wrap = document.createElement("div");
      wrap.className = "tau-rings-wrap";
      wrap.innerHTML = `<svg viewBox="-22 -22 244 244" xmlns="http://www.w3.org/2000/svg">
  <!-- Aro exterior pulso -->
  <circle class="tau-r3" cx="100" cy="100" r="110"
    fill="none" stroke="rgba(198,43,58,.22)" stroke-width="1.2" stroke-dasharray="6 10"/>

  <!-- Aro medio antihorario + 4 nodos blancos -->
  <g class="tau-r2">
    <circle cx="100" cy="100" r="98"
      fill="none" stroke="rgba(255,255,255,.20)" stroke-width="1"
      stroke-dasharray="3 16"/>
    <circle class="tau-nw" cx="100" cy="2"   r="3.5" fill="rgba(255,255,255,.78)"/>
    <circle class="tau-nw" cx="198" cy="100" r="3.5" fill="rgba(255,255,255,.78)"/>
    <circle class="tau-nw" cx="100" cy="198" r="3.5" fill="rgba(255,255,255,.78)"/>
    <circle class="tau-nw" cx="2"   cy="100" r="3.5" fill="rgba(255,255,255,.78)"/>
  </g>

  <!-- Aro interior horario + 4 nodos carmesí -->
  <g class="tau-r1">
    <circle cx="100" cy="100" r="114"
      fill="none" stroke="rgba(255,255,255,.28)" stroke-width="1.5"
      stroke-dasharray="14 7"/>
    <circle class="tau-nd" cx="100" cy="-14" r="5.5" fill="#c62b3a"/>
    <circle class="tau-nd" cx="214" cy="100" r="5.5" fill="#c62b3a"/>
    <circle class="tau-nd" cx="100" cy="214" r="5.5" fill="#c62b3a"/>
    <circle class="tau-nd" cx="-14" cy="100" r="5.5" fill="#c62b3a"/>
  </g>
</svg>`;
      ss.appendChild(wrap);
    }
  ...
  <p class="tau-lema-block__quote">&ldquo;<strong>Hombres nuevos</strong> para <strong>tiempos nuevos</strong>&rdquo;</p>
</div>
...
// Quitar <style> bloques tau anteriores
while (strpos($footer, '.tau-rings-wrap') !== false) {
    $s = strpos($footer, '<style>');
    if ($s === false) break;
    $e = strpos($footer, '</style>', $s);
    if ($e === false) break;
    $footer = substr($footer, 0, $s) . substr($footer, $e + 8);
}
set_config('additionalhtmlfooter', trim($footer) . "\n" . $js, 'theme_moove');

theme_reset_all_caches();
echo "OK v6 — position:relative restaurado, escudo estatico, aros giran\n";
