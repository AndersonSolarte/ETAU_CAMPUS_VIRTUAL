<?php
/**
 * Añade SOLO el CSS del escudo + JS de aros/lema al final del SCSS y footer existentes.
 * NO toca nada del diseño anterior.
 */
define('CLI_SCRIPT', true);
chdir('/var/www/html');
require('/var/www/html/config.php');

// ── CSS del escudo (solo lo mínimo necesario) ─────────────────────────────────
$escudo_css = '
...
.tau-lema{position:relative;text-align:center;overflow:hidden;background:linear-gradient(180deg,#060e24 0%,#091530 70%,#070d20 100%);border-top:2px solid #0d4f8b;padding:16px 1rem 14px;}
.tau-lema::before{content:"";position:absolute;inset:0;pointer-events:none;background-image:radial-gradient(circle,rgba(13,79,139,.09) 1px,transparent 1px);background-size:28px 28px;}
...
.tau-lema-author{font-family:system-ui,Arial,sans-serif;font-size:clamp(.63rem,.88vw,.74rem);color:rgba(255,255,255,.42);letter-spacing:.10em;text-transform:uppercase;margin:4px 0 0;}
...
(function(){
    function go(){
        ...
        if(!document.querySelector(".tau-lema")){
            ...
            lema.innerHTML="<div class=\"tau-lema-row\"><p class=\"tau-lema-q\">“<strong>Hombres nuevos</strong> para <strong>tiempos nuevos</strong>”</p></div><p class=\"tau-lema-author\">Fray Guillermo de Castellana, OFMCap.</p>";
...
// ── Aplicar: SOLO agregar al final, NO limpiar nada ──────────────────────────
$scss = get_config('theme_moove', 'scss');
// Quitar bloque anterior del escudo si existe
$marker = '/* TAU-ESCUDO-HERO ═══';
$p = strpos($scss, $marker);
if ($p !== false) $scss = rtrim(substr($scss, 0, $p));
set_config('scss', $scss . $escudo_css, 'theme_moove');

// Footer: quitar bloques tau-v9 anteriores y agregar
$footer = get_config('theme_moove', 'additionalhtmlfooter');
foreach (['tau-v9-style','tau-v9-script'] as $id) {
    foreach (['<style','<script'] as $tag) {
        while (strpos($footer, $id) !== false) {
            $s = strpos($footer, $tag);
            if ($s === false) break;
...
echo "OK — diseño restaurado + escudo añadido\n";
