...
/* CRÍTICO: position:relative para que .tau-logo-area se posicione aquí */
#mooveslideshow {
    position: relative !important;
    overflow: visible !important;
    min-height: 460px !important;
}
#mooveslideshow .carousel { min-height: 460px !important; }
#mooveslideshow .carousel-inner {
    min-height: 460px !important;
    overflow: hidden !important;
}
#mooveslideshow .carousel-item { min-height: 460px !important; }

/* Logo + lema: wrapper absoluto dentro del hero */
.tau-logo-area {
    ...
    width: clamp(190px,24%,255px) !important;
    animation: tau-area-in .9s cubic-bezier(.22,1,.36,1) both !important;
}
@keyframes tau-area-in {
    from { opacity:0; transform:translateY(-57%) scale(.88); }
...
.tau-hero-lema {
    ...
    background: rgba(20,3,6,.82) !important;
...
.tau-hero-lema-q {
    ...
    font-size: clamp(.73rem,.88vw,.84rem) !important;
...
echo "Position fix aplicado\n";
echo "scss: " . strlen(get_config('theme_moove','scss')) . " bytes\n";
echo "Verificar: #mooveslideshow tiene position:relative\n";
